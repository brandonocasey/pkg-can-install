...
// try the install
console.log(`pkg-can-install: Checking if ${pkg.name} will be installable after publish`);
const result = spawnSync('npm', ['i', '--production'], {cwd: WORKING_DIR, stdio: 'inherit'});

if (result.status === 0) {
  console.log(`pkg-can-install: ${pkg.name} will be installable!`);
} else {
  console.error(`pkg-can-install: ${pkg.name} will not be installable!`);
}
